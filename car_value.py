...
class GetCars:
	...
	def __init__(self, query):
		...
		self.url = 'https://%s.craigslist.org' % self.region
		self.params = {}
	...
	def make_params(self, query):
		try:
			...
			self.params['min_auto_year'] = query['year']
			self.params['max_auto_year'] = query['year']
		except:
			pass
	...
	def get_car_link(self, params):
		...
		for result in rows.find_all('li', {'class': 'result-row'}):
			...
			result_url = info.find('a', {'class': 'result-title'}).get('href')
...
if __name__ == '__main__':
	query = {'region': 'sandiego', 'make': 'toyota', 'model': 'camry', 'year': '2013'}
	myCar = GetCars(query)
